...
class FusionReportLayoutDialog(QDialog):
    ...
    def __init__(
        ...
        self._settings = QSettings("GAMMASYS", "SINCRO_PERFU_FUSION")
    ...
    def _last_dir(default_path: str = "") -> str:
        s = QSettings("GAMMASYS", "SINCRO_PERFU_FUSION")
...
class PerfusionFusionPanel(QDialog):
    ...
    def __init__(self, parent=None):
        ...
        # PERFUSIÓN: sin recon propia, el selector de cortes cambia la VISTA.
        self._cuts_mode_combo.currentIndexChanged.connect(self._on_perfusion_cuts_mode_changed)
        ...
        self._perfusion_stage_combo.addItem("Esfuerzo", "stress")
        ...
        self._btn_fusion_layout.setToolTip(
            "Envía el CT/ATT REGISTRADO de este panel a la etapa elegida del flujo de "
            "perfusión y habilita AC: la próxima 'Recon raw' aplica la corrección física "
            "con este registro."
        )
        ...
        # Cabecera SIEMPRE visible: paciente · ID · fecha · etapa (la setea main).
        self._perfusion_header_lbl = QLabel("")
        self._perfusion_header_lbl.setStyleSheet("color:#fbbf24; font-weight:bold;")
        flow.addWidget(self._perfusion_header_lbl, 0, 0, 1, 10)
        flow.setColumnStretch(9, 1)

        # PERFUSIÓN: SPECT/CT/ATT vienen PRECARGADOS desde la reconstrucción de
        # perfusión — sin botones de carga ni "estudio previo" en este panel.
        for _w in (self._btn_load, self._btn_load_ct, self._btn_load_ct_dir, self._btn_load_att):
            _w.setVisible(False)
        # PERFUSIÓN: la recon vive en el flujo de perfusión (motion+NÍTIDA+filtros
        # por rama) — este panel NO reconstruye: solo registra y fusiona. Las dos
        # recons legítimas son del motor de perfusión (sin AC para registrar, con
        # AC tras 'Registro a AC'). Se conserva el selector de cortes (vista).
        for _w in (self._btn_recon_pipeline, self._btn_cancel_recon, self._preset_combo, self._recon_combo):
            _w.setVisible(False)
        for _pos in ((1, 1), (1, 3)):  # labels "Preset:" y "Recon:"
            _it = flow.itemAtPosition(*_pos)
            if _it is not None and _it.widget() is not None:
                _it.widget().setVisible(False)
        ...
        self._lbl_volume_display.setVisible(False)  # PERFUSIÓN: métrica AMYLO, oculto
        ...
        self._lbl_hmr_result.setVisible(False)  # PERFUSIÓN: resultado AMYLO, oculto
        ...
        hmr_scale_lbl.setVisible(False)  # PERFUSIÓN: escala AMYLO, oculta
        ...
        self._btn_localization_cross.setToolTip(
            ...
            "Para la máscara:\n"
            "1. Posicione la cruz en el centro del corazón\n"
            "2. Click 'Calcular Máscara'"
        )
        ...
        self._btn_loc_anchor.setVisible(False)  # PERFUSIÓN: la cruz viva actúa de ancla
        ...
        self._btn_loc_point_b.setVisible(False)  # PERFUSIÓN: oculto
        ...
        self._btn_loc_clear.setVisible(False)  # PERFUSIÓN: oculto
        loc_btns_row.addWidget(self._btn_loc_clear)
        self._btn_calc_hmr = QPushButton("Calcular Máscara")
        self._btn_calc_hmr.clicked.connect(self._calculate_hmr_spect)
        self._btn_calc_hmr.setToolTip(
            "Segmenta el miocardio (CT anatómica si hay, o VOI esférica) alrededor de la "
            "cruz de Localización y muestra la máscara sobre la fusión."
        )
        ...
        # PERFUSIÓN: control directo de la máscara (ocultar = toggle del check MPR;
        # borrar = reinicio de la segmentación CT, sin confirmación encadenada).
        self._btn_mask_hide = QPushButton("Ocultar máscara")
        self._btn_mask_hide.setCheckable(True)
        self._btn_mask_hide.setToolTip("Muestra/oculta la máscara segmentada sobre la fusión (no la borra).")
        self._btn_mask_hide.toggled.connect(self._on_perfusion_mask_hide_toggled)
        loc_btns_row.addWidget(self._btn_mask_hide)
        self._btn_mask_clear = QPushButton("Borrar máscara")
        self._btn_mask_clear.setToolTip("Borra la segmentación CT completa (máscara, edición manual).")
        self._btn_mask_clear.clicked.connect(self._restart_ct_state)
        loc_btns_row.addWidget(self._btn_mask_clear)
        ...
        # PERFUSIÓN: sin círculos de VOI corazón/mediastino en el MIP; solo máscara.
        try:
            self._mip_widget._chk_vois.setChecked(False)
            self._mip_widget._chk_vois.setVisible(False)
            self._mip_widget._chk_auto_cube.setVisible(False)
        except Exception:
            pass
        ...
        self._qc_split_slider.setMinimumHeight(110)
        ...
        self._fusion_slider.setMinimumHeight(110)
        ...
        self._ct_opacity_slider.setMinimumHeight(110)
        ...
        self._chk_show_vois_mpr.setChecked(False)
        ...
        self._chk_show_vois_mpr.setVisible(False)  # PERFUSIÓN: sin VOIs circulares
        ...
        # PERFUSIÓN: HMR y S/VD son métricas de amiloidosis — se OCULTAN (código
        # intacto para no romper dependencias; la segmentación CT sigue activa).
        self._hmr_group = hmr_group
        hmr_group.setVisible(False)
        ...
        self._btn_export_mask_nifti.setVisible(False)  # PERFUSIÓN: oculto (pedido usuario)
        ...
        ct_gain_col.setSpacing(2)
        _gain_title = QLabel("Brillo CT")
        _gain_title.setStyleSheet("font-size:10px;")
        _gain_title.setAlignment(Qt.AlignmentFlag.AlignCenter)
        ct_gain_col.addWidget(_gain_title)
        self._ct_gain_slider = QSlider(Qt.Orientation.Vertical)
        ...
        ct_gain_col.addWidget(self._ct_gain_slider, 1, Qt.AlignmentFlag.AlignHCenter)
        self._ct_gain_lbl = QLabel("100%")
        self._ct_gain_lbl.setStyleSheet("color:#94a3b8; font-size:9px;")
        self._ct_gain_lbl.setAlignment(Qt.AlignmentFlag.AlignCenter)
        ct_gain_col.addWidget(self._ct_gain_lbl)
        middle_row.addLayout(ct_gain_col)

        # Selectores apilados: Color / Suavizar vista / Ventana CT
        selectors_col = QVBoxLayout()
        selectors_col.setSpacing(4)

        selectors_col.addWidget(QLabel("Color:"))
        ...
        self._spect_cmap_combo.setFixedWidth(95)
        self._spect_cmap_combo.setStyleSheet("font-size:10px; padding:1px 4px;")
        selectors_col.addWidget(self._spect_cmap_combo)

        selectors_col.addWidget(QLabel("Suavizar vista:"))
        ...
        selectors_col.addWidget(self._spect_smooth_spin)

        selectors_col.addWidget(QLabel("Ventana CT:"))
        ...
        selectors_col.addWidget(self._ct_window_combo)
        self._ct_wl_ww_lbl = QLabel("WL 300 / WW 1500")
        self._ct_wl_ww_lbl.setStyleSheet("color:#94a3b8; font-size:9px;")
        selectors_col.addWidget(self._ct_wl_ww_lbl)
        selectors_col.addStretch(1)
        middle_row.addLayout(selectors_col)

        window_vbox.addLayout(middle_row)
    ...
    def _check_startup_auto_continue(self):
        """PERFUSIÓN: sin 'estudio previo' — los datos llegan precargados del flujo."""
        return
    ...
    def _draw_triangulation_cross(self, pix: QPixmap, axis: str) -> QPixmap:
        ...
        try:
            ...
            # Dibujar VOIs si existen. El CONTORNO anatómico del corazón (línea
            # roja de la máscara) sigue la visibilidad de MÁSCARA; los círculos
            # esféricos y el mediastino siguen el toggle de VOIs.
            if (show_vois or _show_mask) and hasattr(self, "_hmr_result") and self._hmr_result is not None:
                hmr_res = self._hmr_result
                spacing = self._spect_spacing_or_default()
                
                # VOI corazón (rojo punteado)
                if hmr_res.voi_heart is not None and (show_vois or isinstance(hmr_res.voi_heart, VOIAnatomical)):
                ...
                # VOI mediastino (azul punteado) — solo con toggle VOIs activo
                if show_vois and hmr_res.voi_mediastinum is not None:
...
""")
    ...
    def _calculate_hmr_spect(self) -> None:
        ...
        try:
            ...
            # PERFUSIÓN: sin botones Fijar A/B — la cruz viva actúa de ancla y el
            # punto B (mediastino, irrelevante para la máscara) se deriva del ancla.
            if anchor is None and getattr(self, "_localization_point_zyx", None) is not None:
                anchor = tuple(int(v) for v in self._localization_point_zyx)
            if anchor is not None and point is None:
                zs = int(np.asarray(self._current_volume).shape[0])
                point = (int(np.clip(int(anchor[0]) - max(4, zs // 4), 0, zs - 1)), int(anchor[1]), int(anchor[2]))
            
            if anchor is None:
                QMessageBox.warning(
                    self, "SINCRO",
                    "Falta ubicar el corazón.\n\n"
                    "1. Active 'Localización'\n"
                    "2. Ctrl+clic en el centro del corazón\n"
                    "3. Click 'Calcular Máscara'"
                )
                return
            ...
            if point is None:
                QMessageBox.warning(
                    self, "SINCRO",
                    "No se pudo derivar el punto de referencia B. Reposicioná la cruz e intentá de nuevo."
                )
                return
            ...
            if (
                ...
                try:
                    ...
                    _prev_seg = getattr(self, "_ct_segmentation", None)
                    try:
                        _prev_voxels = int(np.asarray(_prev_seg.mask_3d).sum()) if _prev_seg is not None else 0
                    except Exception:
                        _prev_voxels = 0
                    # Una máscara VACÍA no se preserva: bloquearía recalcular para siempre.
                    _have_prev_seg = _prev_seg is not None and _prev_voxels > 0
                    _preserve_checked = (
                    ...
                    if hasattr(self, '_btn_toggle_mask_edit'):
                        self._btn_toggle_mask_edit.setEnabled(True)
                        self._brush_radius_spin.setEnabled(True)
                        self._btn_paint_erase.setEnabled(True)
                        self._btn_reset_mask.setEnabled(False)
                        self._btn_apply_mask_edit.setEnabled(False)
                        self._btn_undo_mask.setEnabled(False)
                        if hasattr(self, '_btn_export_mask_nifti'):
            ...
            except Exception as exc_hmr:
                # PERFUSIÓN: el HMR no se usa — si el punto B derivado cayó en fondo
                # vacío, recalcular con mediastino=corazón para no perder la máscara.
                if "mediastino" not in str(exc_hmr).lower():
                    raise
                self._metrics.append(
                    "[MÁSCARA] Mediastino sin señal (punto B en fondo): HMR ignorado — "
                    "la máscara sigue válida."
                )
                result = compute_hmr_spect(
                    volume=analysis_volume,
                    spacing_zyx=spacing,
                    voi_heart=voi_heart,
                    voi_mediastinum=voi_heart,
                    method=method,
                    volume_raw=analysis_volume_raw,
                )
            ...
            # PERFUSIÓN: solo interesa la MÁSCARA — sin métricas HMR/mediastino/PVE
            # (eso es de AMYLO; acá la máscara es QC del registro + VOI anatómico).
            mask_lines = ["\n--- Calcular Máscara ---", f"Tipo VOI: {voi_type_used}"]
            mask_lines.append(f"Ancla A (Z/Y/X): {anchor[0]+1}/{anchor[1]+1}/{anchor[2]+1}")
            _seg = getattr(self, '_ct_segmentation', None)
            if _seg is not None and hasattr(_seg, 'mask_3d'):
                _vox = int(np.asarray(_seg.mask_3d).sum())
                _sp = self._spect_spacing_or_default()
                _ml = _vox * _sp[0] * _sp[1] * _sp[2] / 1000.0
                mask_lines.append(f"Máscara CT: {_vox} voxels · {_ml:.1f} mL")
            else:
                mask_lines.append("Máscara CT: no generada (VOI esférica de respaldo).")
            self._metrics.append("\n".join(mask_lines))
            self._status.setText("Máscara calculada: revisala sobre la fusión (editable con F2.4).")
    ...
    def _blend_mask_over_rgb(rgb: np.ndarray, mask2d: np.ndarray | None, alpha: float) -> np.ndarray:
        ...
        m = PerfusionFusionPanel._mask_to_shape(mask2d, (arr.shape[0], arr.shape[1]))
        a = float(np.clip(alpha, 0.0, 1.0))
        if np.any(m) and a > 0.0:
    ...
    def _invalidate_ct_grid_trial_cache(self):
        ...
        self._trial_ct_native_spacing = None
        self._trial_ref_shape = None
        # Base pesada (resample 2x + auto-flip): también se descarta.
    ...
    def _invalidate_ct_grid_trial_view(self):
        """Solo la vista final (shift/rot/flips): conserva la base pesada y la orientación YA resuelta."""
        self._trial_cache_signature = None
        self._trial_spect_on_ct = None
        self._trial_ct_native = None

    def _calibrate_trial_to_registered(self, ct_native):
        ...
        try:
            ...
            nat_lo = ndi.zoom(np.asarray(ct_native, dtype=np.float32), zf, order=1, prefilter=False)
            nat_view = nat_lo
    ...
    def _build_ct_grid_trial_base(self, base_sig) -> None:
        """Parte PESADA del modo CT nativa: resample 2x + auto-flip, UNA sola vez.

        La orientación queda resuelta acá y NO se recalcula al arrastrar/nudge:
        evita flips espontáneos durante el drag y lo hace mucho más rápido.
        """
        ...
        # PERFUSIÓN: si hay CT registrada, usarla de REFERENCIA para el auto-flip
        # (CT vs CT discrimina mejor y garantiza la MISMA orientación).
        ...
        # IGUAL que la CT registrada (misma verdad para MPR, MIP y máscara).
        ct_native = self._calibrate_trial_to_registered(ct_native)
    ...
    def _ensure_ct_grid_trial_cache(self) -> bool:
        ...
        try:
            # Cache de 2 niveles: la BASE (resample 2x + auto-flip) se reutiliza;
            # el drag/nudge solo re-aplica flips de usuario + rot + shift.
            base_sig = (
                id(self._current_volume),
                id(self._ct_volume),
                bool(getattr(self, "_spect_flip_x_test", False)),
            ...
            # La base está en el marco del REGISTRO: los flips de usuario se aplican
            # al final como DELTAS (idéntico a la CT registrada).
            ...
            # Shift total en px SPECT (64³) + residuo de calibración vs registrada;
            # a px de la grilla objetivo (128³): ×2.
    ...
    def _restart_ct_state(self):
        ...
        # PERFUSIÓN: el check anatómico está en el grupo HMR OCULTO — dejarlo
        # ACTIVO; desactivarlo bloqueaba 'Calcular Máscara' sin forma de volver.
        self._ct_anatomical_check.setChecked(True)
    ...
    def set_perfusion_inputs(self, *, spect_volume, spect_spacing_zyx=None,
                             ct_volume=None, ct_spacing_zyx=None, ct_affine=None,
                             cardiac_axes=None, stage="stress", source_label=""):
        """Precarga SPECT RECONSTRUIDO + CT desde el flujo de perfusión (sin recon acá).

        Registra automáticamente el CT y muestra la FUSIÓN directo (lo que el
        usuario necesita ver); después puede refinar con Ctrl/Shift/nudges.
        """
        from core.amyloid_spect import AmyloidReconstructionBundle
        ...
        self._unfiltered_volume = None
        self._spect_spacing_zyx = tuple(float(v) for v in spect_spacing_zyx) if spect_spacing_zyx else None
        self._bone_mask = None
        self._pre_bone_volume = None
        axes = {}
        for k, a in (cardiac_axes or {}).items():
            if a is None:
                continue
            arr = np.asarray(a, dtype=np.float64)
            if arr.ndim == 4:
                arr = arr.sum(axis=0) if arr.shape[0] > 1 else arr[0]
            axes[k] = np.ascontiguousarray(arr)
        self._recon_bundle = AmyloidReconstructionBundle(
            study=None, source_path=self._current_spect_path, was_raw=False,
            ungated_volume=vol, gated_volume=vol[None, ...],
            tomo_cuts={}, cardiac_axes=axes,
            notes=[f"Volumen precargado desde perfusión ({stage})."],
            spacing_zyx=self._spect_spacing_zyx, affine_ijk_to_lps=None,
        )
        if ct_volume is not None:
            self._ct_volume = np.asarray(ct_volume, dtype=np.float64)
            self._ct_spacing_zyx = tuple(float(v) for v in ct_spacing_zyx) if ct_spacing_zyx else None
            # Affine DICOM del CT: clave para que la vista 'CT nativa' y la CT
            # registrada de baja resolución resuelvan la MISMA orientación.
            self._ct_affine_ijk_to_lps = np.asarray(ct_affine, dtype=np.float64) if ct_affine is not None else None
            if ct_affine is None and hasattr(self, "_metrics"):
                self._metrics.append(
                    "[PERFUSION][WARN] CT sin geometría DICOM (affine): la vista 'CT nativa' "
                    "puede diferir en orientación de la CT registrada. Recargá el CT con el "
                    "botón CT/ATT del flujo de perfusión para adjuntar el affine."
                )
            self._ct_registered = None
            self._ct_auto_registered = None
            self._ct_path = "precargado desde perfusión"
            self._btn_register.setEnabled(True)
        try:
            self._perfusion_stage_combo.blockSignals(True)
            idx = self._perfusion_stage_combo.findData(str(stage))
            if idx >= 0:
        ...
        if ct_volume is not None and hasattr(self, "_ct_anatomical_check"):
            ...
            self._ct_anatomical_check.setChecked(True)
            # setChecked(True) no dispara toggled si ya estaba ON: habilitar a mano.
            self._brush_radius_spin.setEnabled(True)
            self._btn_paint_erase.setEnabled(True)
        self._update_slice_controls()
        self._view_combo.setEnabled(True)
        self._set_combo_by_data(self._view_combo, "cardiac" if axes else "mpr")
        # Selector 'Cortes' del flujo: 'Cardíacos' disponible según ejes de la etapa.
        _c_idx = self._cuts_mode_combo.findData("cardiac")
        if _c_idx >= 0:
            _item = self._cuts_mode_combo.model().item(_c_idx)
            if _item is not None:
                _item.setEnabled(bool(axes))
        self._render_selected_view()
        # Registro automático SOLO en primera carga de la etapa: si el usuario ya
        # ubicó el CT antes, su sesión en memoria manda (no se re-registra).
        if ct_volume is not None:
            try:
                self._status.setText("Registrando CT↔SPECT automáticamente...")
                _t0 = time.perf_counter()
                self._register_ct_to_spect()
                self._metrics.append(f"[PERF] registro automático CT↔SPECT: {time.perf_counter() - _t0:.1f}s")
            except Exception as exc:
                self._status.setText(f"Registro automático falló ({exc}); usá '4. Registrar CT↔SPECT'.")
        else:
            self._status.setText(
    ...
    # Estado completo de una etapa de perfusión (registro, máscara, vistas).
    _PERFUSION_SESSION_ATTRS = (
        "_analysis", "_recon_bundle", "_current_volume", "_base_spect_volume", "_unfiltered_volume",
        ...
        "_bone_mask", "_pre_bone_volume",
        "_spect_view_offset", "_ct_view_offset", "_spect_pan_px", "_ct_pan_px",
    ...
    def _save_perfusion_stage_session(self, stage: str):
        sess = {name: getattr(self, name, None) for name in self._PERFUSION_SESSION_ATTRS}
        try:
            sess["__nudge"] = (float(self._nudge_z.value()), float(self._nudge_y.value()), float(self._nudge_x.value()))
            sess["__rot"] = (float(self._rot_z.value()), float(self._rot_y.value()), float(self._rot_x.value()))
        except Exception:
            pass
        self._perfusion_stage_sessions[str(stage)] = sess

    def _restore_perfusion_stage_session(self, stage: str) -> bool:
        sess = self._perfusion_stage_sessions.get(str(stage))
        if not sess:
            return False
        ...
        try:
            for spin, val in zip((self._nudge_z, self._nudge_y, self._nudge_x), sess.get("__nudge", (0.0, 0.0, 0.0))):
                ...
                spin.blockSignals(False)
            for spin, val in zip((self._rot_z, self._rot_y, self._rot_x), sess.get("__rot", (0.0, 0.0, 0.0))):
                spin.blockSignals(True)
                spin.setValue(float(val))
                spin.blockSignals(False)
        except Exception:
            pass
        # Cachés dependientes del estado: invalidar y re-renderizar.
        self._invalidate_ct_grid_trial_cache()
        self._mip_vol_cache_sig = None
        self._update_slice_controls()
        self._render_selected_view()
        self._status.setText(
            f"Etapa {('ESFUERZO' if stage == 'stress' else 'REPOSO')} restaurada de memoria "
            "(registro, máscara y vistas intactos)."
        )
        return True

    def update_perfusion_spect(self, stage: str, spect_volume, spect_spacing_zyx=None):
        """Reemplaza SOLO el SPECT (p.ej. tras re-recon con AC) sin tocar el registro CT."""
        vol = np.asarray(spect_volume, dtype=np.float64)
        stage = str(stage)
        sess = self._perfusion_stage_sessions.get(stage)
        if sess is not None:
            sess["_current_volume"] = vol
            sess["_base_spect_volume"] = vol.copy()
            if spect_spacing_zyx:
                sess["_spect_spacing_zyx"] = tuple(float(v) for v in spect_spacing_zyx)
        if self._perfusion_current_stage == stage:
            self._base_spect_volume = vol.copy()
            self._current_volume = vol
            if spect_spacing_zyx:
                self._spect_spacing_zyx = tuple(float(v) for v in spect_spacing_zyx)
            self._invalidate_ct_grid_trial_cache()
            self._mip_vol_cache_sig = None
            self._update_slice_controls()
            self._render_selected_view()
            self._status.setText("SPECT actualizado (recon nueva); registro CT conservado.")

    def set_perfusion_header(self, stage: str, text: str):
        """Guarda y muestra la cabecera de paciente/etapa (por etapa)."""
        self._perfusion_stage_headers[str(stage)] = str(text or "")
        cur = self._perfusion_current_stage
        if cur is None or cur == str(stage):
            self._apply_perfusion_header(str(stage))

    def _apply_perfusion_header(self, stage: str):
        txt = self._perfusion_stage_headers.get(str(stage)) or ""
        try:
            self._perfusion_header_lbl.setText(txt)
            if txt:
                self.setWindowTitle(f"SINCRO — Fusión SPECT/CT · PERFUSIÓN — {txt}")
        except Exception:
            pass

    def _on_perfusion_cuts_mode_changed(self, _idx: int = 0):
        """En perfusión el selector 'Cortes' cambia la vista (acá no hay recon)."""
        if self._recon_bundle is None:
            return
        mode = str(self._cuts_mode_combo.currentData() or "mixed")
        axes = getattr(self._recon_bundle, "cardiac_axes", None) or {}
        if mode == "cardiac":
            if axes:
                self._set_combo_by_data(self._view_combo, "cardiac")
            else:
                self._status.setText(
                    "Cortes cardíacos: esta etapa no trae ejes cardíacos — generá los cortes "
                    "(reorientación) en el flujo de perfusión y reabrí la fusión."
                )
                self._set_combo_by_data(self._cuts_mode_combo, "mixed")
        elif mode == "tomo":
            self._set_combo_by_data(self._view_combo, "tomo")
        else:
            self._set_combo_by_data(self._view_combo, "mpr")

    def _on_perfusion_stage_changed(self, _idx: int):
        """Cambio de etapa: restaura la sesión en memoria; solo pide datos si no hay."""
        new_stage = str(self._perfusion_stage_combo.currentData() or "stress")
        prev = self._perfusion_current_stage
        if prev and prev != new_stage:
            self._save_perfusion_stage_session(prev)
    ...
    def _on_perfusion_mask_hide_toggled(self, hidden: bool):
        ...
        try:
            self._chk_show_mask_mpr.setChecked(not bool(hidden))
        except Exception:
            pass
    ...
    def _apply_ct_nudge(self):
        ...
        # Nudge: invalidar SOLO la vista del modo CT nativa (la base y la
        # orientación resuelta se conservan — evita flips espontáneos y es rápido).
        self._invalidate_ct_grid_trial_view()
        if abs(rot[0]) > 1e-6:
